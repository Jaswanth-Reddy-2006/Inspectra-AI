...
export default function PerfAccessibility({ result }) {
    ...
    useEffect(() => {
        if (result?.perfAccessibility) {
            setAnalysisResult(result.perfAccessibility);
        } else if (result?.performance) {
            // Handle cases where the prop might be the content directly
    ...
    // Auto-run analysis when URL is present and no results exist
    useEffect(() => {
        if (url && !analysisResult && !running && !error && !globalScanning) {
            handleAnalyze();
        }
    }, [url, analysisResult, running, error, globalScanning]);

    const handleAnalyze = async () => {
        if (!url) return;
        setRunning(true); setAnalysisResult(null); setError(null);
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (ev.type === 'result') setAnalysisResult(ev.result);
    ...
    const radarData = analysisResult ? [
        { metric: 'Performance', value: analysisResult.scores.performance },
        { metric: 'Accessibility', value: analysisResult.scores.accessibility },
        { metric: 'Best Practice', value: analysisResult.scores.bestPractices },
        { metric: 'LCP', value: analysisResult.vitals.lcp.score },
        { metric: 'CLS', value: analysisResult.vitals.cls.score },
    ] : [];
        ...
        <div className="flex flex-col h-full min-h-[calc(100vh-64px)] bg-[#f8fafc]">
            ...
            <div className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2.5 shrink-0">
                    ...
                    <div>
                        <p className="text-sm font-black text-slate-800">Testing Performance</p>
                ...
                {running && (
                    <div className="flex items-center gap-2 bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-xs font-black">
                        ...
                        <AnimatePresence mode="wait">
                                ...
                                <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-5">
                                    ...
                                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                                        <div className="flex flex-wrap items-center justify-around gap-8">
                                            {/* Overall */}
                                            <div className="flex flex-col items-center gap-2">
                                                <ScoreGauge score={analysisResult.scores.overall} label="Overall" size="lg" />
                                                <p className="text-[10px] font-semibold text-slate-400">Combined Score</p>
                                            </div>
                                            <div className="w-px h-24 bg-slate-100 hidden md:block" />
                                            <ScoreGauge score={analysisResult.scores.performance} label="Performance" icon={Zap} size="lg" />
                                            <ScoreGauge score={analysisResult.scores.accessibility} label="Accessibility" icon={Eye} size="lg" />
                                            <ScoreGauge score={analysisResult.scores.bestPractices} label="Best Practices" icon={ShieldCheck} size="lg" />
                                            <div className="w-px h-24 bg-slate-100 hidden md:block" />
                                            {/* Radar */}
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                            {Object.entries(analysisResult.vitals).map(([key, vital]) => (
                                    ...
                                    {analysisResult.screenshot && (
                                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                            <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-slate-600">{analysisResult.title}</span>
                                            </div>
                                            <img src={analysisResult.screenshot} alt="Page screenshot" className="w-full" />
                                        </div>
                                    )}
                                </motion.div>
                            )}
                            ...
                            {activeTab === 'perf' && (
                                <motion.div key="perf" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                                    {/* Slow resources */}
                                    <IssueAccordion title="Slow Resources (> 500ms)" icon={Clock} count={analysisResult.slowResources?.length || 0}
                                        color="#f59e0b" bg="#fef3c7" defaultOpen>
                                        <div className="space-y-2">
                                            {analysisResult.slowResources?.map((r, i) => (
                                    ...
                                    {/* Layout shifts */}
                                    <IssueAccordion title="Layout Shifts (CLS)" icon={LayoutGrid} count={analysisResult.clsDetails?.length || 0}
                                        color="#ef4444" bg="#fee2e2">
                                        <div className="space-y-2">
                                            {analysisResult.clsDetails?.map((cls, i) => (
                                                <div key={i} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                                    ...
                                    {/* Console errors */}
                                    <IssueAccordion title="Console Errors" icon={AlertTriangle} count={analysisResult.consoleErrors?.length || 0}
                                        color="#ef4444" bg="#fee2e2">
                                        <div className="space-y-1.5">
                                            {analysisResult.consoleErrors?.map((e, i) => (
                            ...
                            {activeTab === 'a11y' && (
                                <motion.div key="a11y" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                                    {/* Axe summary */}
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        {Object.entries(analysisResult.axeSummary.byImpact).map(([impact, count]) => {
                                    ...
                                    {/* Missing labels */}
                                    <IssueAccordion title="Missing / Mismatched Labels" icon={AlignLeft} count={analysisResult.missingLabels?.length || 0}
                                        color="#6366f1" bg="#eef2ff" defaultOpen>
                                        <div className="space-y-2">
                                            {analysisResult.missingLabels?.map((m, i) => (
                                    ...
                                    {/* Contrast failures */}
                                    <IssueAccordion title="Color Contrast Failures" icon={Contrast} count={analysisResult.contrastFails?.length || 0}
                                        color="#ec4899" bg="#fce7f3">
                                        <div className="space-y-2">
                                            {analysisResult.contrastFails?.map((c, i) => (
                                                <div key={i} className="bg-slate-50 rounded-xl p-3">
                                    ...
                                    {/* ARIA violations */}
                                    <IssueAccordion title="ARIA Violations" icon={Eye} count={analysisResult.ariaViolations?.length || 0}
                                        color="#8b5cf6" bg="#ede9fe">
                                        <div className="space-y-2">
                                            {analysisResult.ariaViolations?.map((v, i) => (
                                    ...
                                    {/* Tab navigation */}
                                    <IssueAccordion title="Tab Navigation Issues" icon={Navigation} count={analysisResult.tabViolations?.length || 0}
                                        color="#f97316" bg="#fff7ed">
                                        <div className="space-y-2">
                                            {analysisResult.tabViolations?.map((v, i) => (
                                                <div key={i} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                                    ...
                                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                                        <div className="px-5 py-4 border-b border-slate-100">
                                            <p className="text-sm font-black text-slate-800">All Violations ({analysisResult.axeSummary.violations})</p>
                                        </div>
                                        <div className="overflow-x-auto">
                                            <table className="w-full text-xs">
                                                ...
                                                <tbody>
                                                    {analysisResult.axeSummary.allViolations?.map((v, i) => (
                                ...
                                <motion.div key="bp" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
                                    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                            {[
                                                { l: 'HTTPS', v: analysisResult.bestPracticesSignals?.isHTTPS },
                                                { l: 'Meta Description', v: analysisResult.bestPracticesSignals?.hasMetaDesc },
                                                { l: 'Viewport Meta', v: analysisResult.bestPracticesSignals?.hasViewport },
                                                { l: 'Charset', v: analysisResult.bestPracticesSignals?.hasCharset },
                                                { l: 'Favicon', v: analysisResult.bestPracticesSignals?.hasFavicon },
                                                { l: 'Lang Attribute', v: analysisResult.bestPracticesSignals?.hasLangAttr },
                                            ].map(s => (
                                        ...
                                        {analysisResult.bestPracticesSignals?.imagesTotal > 0 && (
                                            <div className="mt-4 p-4 bg-slate-50 rounded-xl">
                                                <p className="text-xs font-bold text-slate-600 mb-1">Images without alt text</p>
                                                <p className="text-2xl font-black text-red-500">{analysisResult.bestPracticesSignals.imgWithoutAlt}</p>
                                                <p className="text-[10px] text-slate-400">of {analysisResult.bestPracticesSignals.imagesTotal} total images</p>
                                            </div>
                                        )}
                                        {analysisResult.bestPracticesSignals?.consoleErrors > 0 && (
                                            <div className="mt-3 p-4 bg-red-50 rounded-xl">
                                                <p className="text-xs font-bold text-red-600 mb-1">Console Errors</p>
                                                <p className="text-2xl font-black text-red-500">{analysisResult.bestPracticesSignals.consoleErrors}</p>
                                            </div>
                                        )}
                                    </div>
                                </motion.div>
