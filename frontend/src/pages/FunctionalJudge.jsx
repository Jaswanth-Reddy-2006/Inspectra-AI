...
export default function FunctionalJudge({ result }) {
    ...
    useEffect(() => {
        if (result?.functionalTests) {
            setResults(result.functionalTests);
        }
    }, [result]);
    ...
    // Auto-run flows when URL is present and no results exist
    useEffect(() => {
        if (url && Object.keys(results).length === 0 && !running && !error && !globalScanning && selectedFlows.size > 0) {
            handleRun();
        }
    }, [url, results, running, error, globalScanning, selectedFlows.size]);
    ...
    return (
        <div className="flex flex-col h-full min-h-[calc(100vh-64px)] bg-[#f8fafc]">
            ...
            <div className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm flex items-center gap-3 flex-wrap">
                ...
                {running && (
                    <div className="flex items-center gap-2 bg-violet-50 text-violet-600 px-4 py-2 rounded-xl text-xs font-black">
