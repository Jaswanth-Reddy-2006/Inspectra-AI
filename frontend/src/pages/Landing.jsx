...
const Landing = () => {
    ...
    return (
        <div className="bg-white min-h-screen font-sans selection:bg-blue-100 selection:text-blue-700">
            ...
            {/* Live System Health */}
            <section id="demo" className="py-16 sm:py-24 lg:py-32 px-5 sm:px-8 scroll-mt-20">
                <div className="max-w-7xl mx-auto text-center mb-10 sm:mb-16 lg:mb-20 space-y-3 sm:space-y-4">
                    <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight leading-none">Live System Health</h2>
                    <p className="text-slate-500 font-medium max-w-xl mx-auto text-sm sm:text-base">Monitor your product quality in real-time with agentic feedback loops.</p>
                </div>

                <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-5 sm:gap-6">
                    {/* Main Chart */}
                    <div className="lg:col-span-3 bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-slate-100 shadow-2xl relative overflow-hidden">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-10 gap-3">
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest underline decoration-2 decoration-blue-200">Last scan: 2 min ago</p>
                        </div>

                        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16 lg:gap-24">
                            {/* Circle */}
                            <div className="relative w-40 h-40 sm:w-52 sm:h-52 md:w-64 md:h-64 shrink-0">
                                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                                    <circle className="text-slate-100 fill-none stroke-current" strokeWidth="8" cx="50" cy="50" r="42" />
                                    <circle className="text-blue-600 fill-none stroke-current" strokeWidth="8" strokeDasharray="263.8" strokeDashoffset="5" strokeLinecap="round" cx="50" cy="50" r="42" />
                                </svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900">98%</span>
                                    <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Health Score</span>
                                </div>
                            </div>

                            {/* Severity Bars */}
                            <div className="flex-1 w-full space-y-5 sm:space-y-8">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Severity Distribution</p>
                                <div className="space-y-4 sm:space-y-6">
                                    {[
                                        { label: 'Critical', value: 0, color: 'bg-red-500', w: '0%' },
                                        { label: 'Major', value: 4, color: 'bg-orange-500', w: '15%' },
                                        { label: 'Minor', value: 26, color: 'bg-blue-600', w: '45%' }
                                    ].map((item, i) => (
                                ...
                                <div className="pt-3 flex items-center gap-2 text-emerald-500 font-black text-xs sm:text-sm uppercase italic tracking-widest">
                    ...
                    {/* Stat cards */}
                    <div className="lg:col-span-1 grid grid-cols-2 lg:grid-cols-1 gap-5 sm:gap-6">
                        {[
                            { label: 'Active Test Suites', value: '1,240', sub: '8% Increase', subColor: 'text-emerald-500', icon: <CheckCircle2 size={12} /> },
                            { label: 'Avg. Agent Fidelity', value: '99.9%', sub: 'High Reliability', subColor: 'text-blue-500', icon: <Globe size={12} /> }
                        ].map((stat, i) => (
                            <div key={i} className="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-xl flex-1 flex flex-col justify-center">
                                <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 sm:mb-4">{stat.label}</p>
                                <p className="text-3xl sm:text-[44px] lg:text-[56px] font-black text-slate-900 mb-1 sm:mb-2 leading-none">{stat.value}</p>
                                <p className={`text-[10px] sm:text-[11px] font-bold flex items-center gap-2 uppercase tracking-tight ${stat.subColor}`}>
                                    {stat.icon} {stat.sub}
                                </p>
                            </div>
                        ))}
