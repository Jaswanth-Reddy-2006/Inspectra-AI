...
export default function VisualJudge() {
    const { targetUrl: url, baselineUrl: globalBaseline, isScanning: globalScanning } = useScanContext();
    const [baselineUrl, setBaselineUrl] = useState(globalBaseline || '');
    ...
    useEffect(() => {
        if (globalBaseline) setBaselineUrl(globalBaseline);
    }, [globalBaseline]);
    ...
    // Auto-run comparison when both URLs are present and no results exist
    useEffect(() => {
        if (baselineUrl && currentUrl && !result && !running && !error && !globalScanning) {
            handleCompare();
        }
    }, [baselineUrl, currentUrl, result, running, error, globalScanning]);
        ...
        <div className="flex flex-col h-full min-h-[calc(100vh-64px)] bg-[#f8fafc]">
            ...
            <div className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm flex flex-wrap items-center gap-3">
                ...
                <div className="flex-1 flex items-center gap-3 min-w-[300px]">
                    <div className="flex-1 flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 focus-within:border-pink-300 transition-all">
                        <Globe size={11} className="text-slate-400 shrink-0" />
                        <input type="url" value={baselineUrl} onChange={e => setBaselineUrl(e.target.value)} placeholder="Baseline URL (e.g. Production Site)" disabled={running}
                            className="flex-1 bg-transparent text-xs font-mono text-slate-800 placeholder-slate-400 outline-none" />
                    </div>
                    <div className="flex items-center gap-2 px-3 py-2 bg-pink-50 border border-pink-100 rounded-xl shrink-0">
                        <ArrowLeftRight size={11} className="text-pink-400" />
                        <span className="text-[10px] font-black text-pink-700 uppercase tracking-widest">Comparing Against: {url || 'None'}</span>
                    </div>
                </div>
                ...
                {running && (
                    <div className="flex items-center gap-2 bg-pink-50 text-pink-600 px-4 py-2 rounded-xl text-xs font-black">
