    ...
    return (
        <div className="flex flex-col h-full min-h-[calc(100vh-64px)] bg-[#f8fafc]">
            ...
            <div className="bg-white border-b border-slate-100 px-6 py-4 shadow-sm flex items-center gap-3 flex-wrap">
                ...
                <button onClick={handleMonitor} disabled={!url || running}
                    className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white px-5 py-2.5 rounded-xl text-sm font-black transition-all shadow-lg shadow-indigo-200 hover:scale-[1.02] active:scale-95 whitespace-nowrap">
                    {running ? <><Loader size={13} className="animate-spin" />{progress.phase}…</> : <><Activity size={13} />Monitor</>}
                </button>
