                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 scrollbar-thin scrollbar-thumb-slate-200">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Remediation Roadmap</h3>
                        </div>
                        <div className="space-y-3">
                            {recommendations.map((rec, i) => (
                                <div key={i} className={`p-5 rounded-3xl border ${rec.bg} ${rec.border} flex items-start gap-4`}>
                                    <div className={`w-10 h-10 rounded-2xl bg-white border ${rec.border} flex items-center justify-center shrink-0 shadow-sm`}>
                                        <rec.icon size={18} className={rec.color} />
                                    </div>
                                    <div>
                                        <p className="text-xs font-black text-slate-900 uppercase tracking-tight mb-1">{rec.title}</p>
                                        <p className="text-[11px] font-medium text-slate-500 leading-relaxed">{rec.text}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-4">
                            {SEVERITY_GROUPS.map(group => (
                                <IssueGroup key={group} label={group} issues={grouped[group]} />
                            ))}
                        </div>
                    </div>
